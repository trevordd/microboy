...
//
@implementation MenuOptionsTimeLayer
...
-(id) init{
   ...
   // [self addChild:menu2BG z:0];
    ...
    //
    titleLabel = [CCLabelTTF labelWithString:@"MATCH LENGTH" fontName:@MENU_FONT fontSize:MENU_TITLE_FONT_SIZE];
    ...
    //
//    time1mins = [CCButton buttonWithTitle:@"1 MIN" fontName:@MENU_FONT fontSize:MENU_FONT_SIZE];
//    [time1mins setPosition: ccp(menuX +(0*100),menuYBase-(0*30))];
//    [time1mins setTarget:self selector:@selector(on1mins:)];
//    [self addChild: time1mins];

    time2mins = [CCButton buttonWithTitle:@"2 MINS" fontName:@MENU_FONT fontSize:MENU_FONT_SIZE];
    [time2mins setPosition: ccp(menuX +(0*100),menuYBase-(0*30))];
    [time2mins setTarget:self selector:@selector(on2mins:)];
    [self addChild: time2mins];

    time3mins = [CCButton buttonWithTitle:@"3 MINS" fontName:@MENU_FONT fontSize:MENU_FONT_SIZE];
    [time3mins setPosition: ccp(menuX +(0*100),menuYBase-(1*30))];
    [time3mins setTarget:self selector:@selector(on3mins:)];
    [self addChild: time3mins];
   
    time4mins = [CCButton buttonWithTitle:@"4 MINS" fontName:@MENU_FONT fontSize:MENU_FONT_SIZE];
    [time4mins setPosition: ccp(menuX +(0*100),menuYBase-(2*30))];
    [time4mins setTarget:self selector:@selector(on4mins:)];
    [self addChild: time4mins];

    time5mins = [CCButton buttonWithTitle:@"5 MINS" fontName:@MENU_FONT fontSize:MENU_FONT_SIZE];
    [time5mins setPosition: ccp(menuX +(0*100),menuYBase-(3*30))];
    [time5mins setTarget:self selector:@selector(on5mins:)];
    [self addChild: time5mins];
    
    time7mins = [CCButton buttonWithTitle:@"7 MINS" fontName:@MENU_FONT fontSize:MENU_FONT_SIZE];
    [time7mins setPosition: ccp(menuX +(0*100),menuYBase-(4*30))];
...
-(void) resetButtonColors {
    ...
    [time2mins setColor:[CCColor whiteColor]];
    [time3mins setColor:[CCColor whiteColor]];
    [time4mins setColor:[CCColor whiteColor]];
    [time5mins setColor:[CCColor whiteColor]];
    [time7mins setColor:[CCColor whiteColor]];
...
-(void) updateButtonColors {
    ...
    switch ([GameData getMatchLength]) {
        ...
        case MINS_4 :
            [time4mins setColor:[CCColor yellowColor]];
            break;
...
- (void)on4mins:(id)sender
{
    ...
    [GameData setMatchLength:MINS_4];
    [self updateButtonColors];
}
